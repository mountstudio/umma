        ...
        <div class="col-12 col-sm-10 col-lg-10 col-md-10">
            ...
            <form action="{{ route('admin.questions.update', $question) }}" method="POST">
                ...
                <div id="categories">
                    ...
                    <select id="categories" name="category_id">
                        @foreach($questionCategories as $questionCategory)
                            <option value="{{ $questionCategory->id }}" {{ $question->category_id == $questionCategory->id ? 'selected' : '' }}>{{ $questionCategory->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-check">
                    <input type="checkbox" name="is_anonim" {{ $question->is_anonim ? 'checked':'' }} class="form-check-input" id="is_anonim_check">
                ...
                <div class="form-group">
                    ...
                    <select id="user_select" name="user_id">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}"{{ $question->user_id == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group {{$question->user ? 'd-none':''}}" id="user_form_div">
                    ...
                    <input type="text" value="{{ $question->full_name }}" class="form-control" id="full_name_input" name="full_name" required>
                    <label for="phone_input">Телефон:<span class="text-danger">*</span></label>
                    <input type="text" value="{{ $question->phone }}" class="form-control" id="phone_input" name="phone" required>
                    <label for="mail_input">Почта:<span class="text-danger">*</span></label>
                    <input type="text" value="{{ $question->mail }}" class="form-control" id="mail_input" name="mail" required>
                </div>
