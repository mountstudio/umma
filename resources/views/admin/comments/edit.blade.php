        ...
        <div class="col-12 col-sm-10 col-lg-10 col-md-10">
            <form action="{{ route('admin.comments.update', $comment) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select id="article" name="article_id">
                        @foreach($articles as $article)
                            <option value="{{ $article->id }}"{{ $comment->article_id == $article->id ? 'selected' : '' }}>{{ $article->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group {{$comment->user ? 'd-none':''}}" id="user_form_div">
                    ...
                    <input type="text" value="{{ $comment->phone }}" class="form-control" id="phone_input" name="phone"
                           required>
