<a href="{{ route('show.poster', $poster) }}" class="card text-dark border-0 shadow-sm text-decoration-none h-100 mb-2">
    ...
    <div class="card-body shadow-sm border border-orange p-0 w-50 bg-white text-center rounded"
        ...
        <p class="pt-2 h4 mb-0" style="line-height: 75%;">
            <span class="text-orange font-weight-bold h3"
                  style="    text-shadow: 0 .1rem .2rem rgba(213, 126, 42,.35)!important;">{{ \Carbon\Carbon::make($poster->date_event)->formatLocalized('%d') }}</span>
            <br>
            <span class="small text-capitalize">{{ \Carbon\Carbon::make($poster->date_event)->formatLocalized('%b') }}</span>
        </p>
